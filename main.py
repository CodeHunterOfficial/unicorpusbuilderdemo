# main.py — Universal entry point for the UniCorpusBuilder pipeline
"""
UniCorpusBuilder — single entry point for all operations.
...
# Mapping of social platform names to scraper entry points
SOCIAL_SCRIPTS = {
...
def cmd_config(*args):
    """Print resolved site configuration and language package summary."""
    if not args:
    ...
    # Print language package statistics
    ...
    # Print resolved author regex patterns (universal + language)
...
def cmd_pipeline(*args):
    """Run the full news pipeline (discovery + extraction) for a single site."""
    if not args:
...
def cmd_bulk(*args):
    """Run the pipeline sequentially on all sites of a given language."""
    if not args:
    ...
    target_sites = (
        sites if language == "all"
        else {k: v for k, v in sites.items()
              if v.get("default_language") == language}
    )
...
def cmd_corpus(*args):
    """Build a unified corpus from collected JSONL files."""
...
def cmd_social(*args):
    """Dispatch to social scraper entry points (VK, Telegram, Rutube)."""
    if not args:
    ...
    if platform == "all":
        # Run all social scrapers sequentially
...
def cmd_wiki(*args):
    """Run the Wikipedia dump parser."""
    script = "wiki/wiki_dump_parser.py"
    if not args:
        print("Usage: python main.py wiki <dump_url> [output_dir] "
              "[--max-articles N] [--min-length N] [--transliterate]")
        return
...
def cmd_docs(*args):
    """Run the universal document parser (PDF, DOCX, HTML, TXT)."""
...
def cmd_build_vk(*args):
    """Build a cleaned unified VK dataset from collected posts and comments."""
...
HELP = """
UniCorpusBuilder — multilingual news + social + wiki + docs scraper
